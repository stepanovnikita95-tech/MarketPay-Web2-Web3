...
contract MarketPay is Ownable, ReentrancyGuard, Pausable{
    ...
    function addTokenApproval(address _token) external onlyOwner whenNotPaused {
    ...
    function removeTokenApproval(address _token) external onlyOwner whenNotPaused {
        require(_token != address(0), ZeroAddress());
    ...
    function withdrawal(address _token, address _to, uint256 _amount) external nonReentrant onlyOwner whenNotPaused{
        require(_token != address(0), ZeroAddress());
    ...
    function withdrawalETH(address payable _to, uint256 _amount) external nonReentrant onlyOwner whenNotPaused {
        require(_to != address(0), ZeroAddress());
        require(_amount > 0, ZeroAmount());
    ...
    function withdrawalAll(address _token, address _to) external nonReentrant onlyOwner {
        require(_token != address(0), ZeroAddress());
        require(_to != address(0), ZeroAddress());
    ...
    function withdrawalAllETH(address payable _to) external nonReentrant onlyOwner {
